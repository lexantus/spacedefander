package game.objs
{
	...
	public class GameActor extends Stage3dEntity
	{
		...
		public var tintVerlocity:Vector3D;
				...
				if(age >= shootNext)
				{
					...
					if(shootDist < 0)
						shouldShoot = true;
							...
							(shootDist > 0) &&
							(Vector3D.distance(position, shootAt.position) <= shootDist))
					{
						shouldShoot = true;
					}
		...
		public function cloneactor():GameActor
		{
			...
			myclone.tintVerlocity = this.tintVerlocity;
		...
		// used for collistion callback performed in GameActorpool
		...
		private function aabbCollision(min1:Vector3D, max1:Vector3D,
		...
		public function isCollidingAabb(checkme:GameActor):Boolean
		{
			...
			if(aabbCollision(position+aabbMin,
						     position+aabbMax,
